...
const sourceFile = path.join(__dirname, '..', 'output_log.txt');
...
getAuthKey = async() => {
    const text = await fsPromises.readFile(sourceFile, 'utf-8');
...
const getGachaLog = async ({ wish, page, endId }) => {
    try {
        const params = {
            "authkey_ver": 1,
            "lang": "en",
...
const getGachaLogs = async (wish) => {
    try {
        ...
        do {
            ...
            list = await getGachaLog({ wish, page, endId });
            result.push(...list);
            page += 1;
            if (result.length > 0) {
...
module.exports = async () => {
    try {
        ...
        for (let x = 0; x < wishes.length; x++) {
            const gachaLogs = await getGachaLogs(wishes[x]);
            // Sorted in ASC mode
